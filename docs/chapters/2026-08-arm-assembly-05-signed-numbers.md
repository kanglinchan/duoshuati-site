...
注意一个不对称的细节：负数比正数多一个。原因是零占了正数那半边（`0x00000000` 算"非负"），所以正数只有 $2^{31}-1$ 个、负数有 $2^{31}$ 个。这就引出一个尴尬的边界情况：`0x80000000`（即 `-2147483648`）取反加一得到它自己——理论上它的"正数对应物" `+2147483648` 在 32 位里根本存不下。所以这条指令
